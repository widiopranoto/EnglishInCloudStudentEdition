$(document).ready( function() {
  ...
  var the_phrase = shuffle($('#hfData1').val());
  var orig       = $('#hfData1').val().replace(/([\,\.])/g," $1").split(" "); // save it, so we have access to it when doing tips

  //
  $.each(the_phrase, function(i,x) {
    var x_nice = x.replace(/\-/g," ").replace(/:::/g,"-");
    if (hasUppercase(x)) {
      $("#jumbleStarter1").append("<label class='jumbled_word' data-upper=\"yes\" data-real-value="+x+">"+x_nice+"</label>");
    } else {
      $("#jumbleStarter1").append("<label class='jumbled_word' data-real-value="+x+">"+x_nice+"</label>");
    }
  });
  $("#jumbleStarter1").append("<div style=\"clear:both\"></div>");
  ...
  $("#jumbleStarter1").sortable({
        connectWith: '#jumbleWords',
        items: '.jumbled_word',
        appendTo: 'body', helper: 'clone', zIndex: 300
    });

//   $("#jumbleWords1").change( function() {
...
//    ucword
    ...
    $('#btnCheckScore').click(function(event) {
        ...
        var original_str = $('#hfData1').val().replace(/([\,\.])/g," $1");
...
function get_correct_answers() {
  ...
  var correct = new Array();;
  correct.push($('#hfData1').val().replace(/([\,\.])/g," $1").replace(/-/g," ").replace(/:::/g,"-"));
  if ($('#hfData2').length) {
