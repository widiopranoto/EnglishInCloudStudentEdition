$(document).ready( function() {
  ...
  var the_phrase = shuffle($('#hfDataB').val());
  var orig       = $('#hfDataB').val().replace(/([\,\.])/g," $1").split(" "); // save it, so we have access to it when doing tips

  //
  $.each(the_phrase, function(i,x) {
    var x_nice = x.replace(/\-/g," ").replace(/:::/g,"-");
    if (hasUppercase(x)) {
      $("#jumbleStarterB").append("<label class='jumbled_word' data-upper=\"yes\" data-real-value="+x+">"+x_nice+"</label>");
    } else {
      $("#jumbleStarterB").append("<label class='jumbled_word' data-real-value="+x+">"+x_nice+"</label>");
    }
  });
  $("#jumbleStarterB").append("<div style=\"clear:both\"></div>");

  var div_height = $('#jumbleStarterB').height();

  //console.log("height: "+ $('#jumbleStarterB').height());
  $('#jumbleWordsB,#jumbleStarterB').height( div_height + "px" ); // set the heights

  //alert("Set height to " +div_height );

  $("#jumbleWordsB").sortable({
        connectWith: '#jumbleStarterB',
        items: '.jumbled_word',
        helper: 'clone',
        appendTo: 'body', helper: 'clone', zIndex: 300,
        stop: function( event, ui ) {
          $('#jumbleWordsB label').each(function(i) {
  ...
  $("#jumbleStarterB").sortable({
        connectWith: '#jumbleWordsB',
        items: '.jumbled_word',
        appendTo: 'body', helper: 'clone', zIndex: 300
    });

//   $("#jumbleWordsB").change( function() {
...
//    ucword
    ...
    $('#btnCheckScore').click(function(event) {
        ...
        var original_str = $('#hfDataB').val().replace(/([\,\.])/g," $1");
        ...
        if ($('#hfDataB2').length) {
          original_str_poss2 = $('#hfDataB2').val().replace(/([\,\.])/g," $1");
        }
        if ($('#hfDataB3').length) {
          original_str_poss3 = $('#hfDataB3').val().replace(/([\,\.])/g," $1");
        }
        if ($('#hfDataB4').length) {
          original_str_poss4 = $('#hfDataB4').val().replace(/([\,\.])/g," $1");
        }
        if ($('#hfDataB5').length) {
          original_str_poss5 = $('#hfDataB5').val().replace(/([\,\.])/g," $1");
        }
        if ($('#hfDataB6').length) {
          original_str_poss6 = $('#hfDataB6').val().replace(/([\,\.])/g," $1");
        }

        // get the values they have put in already
        var the_string = new Array();
        $('#jumbleWordsB .jumbled_word').each(function(index, el) {
...
// console.log(the_string.join(" ") + " == "+ original_str_poss6);
    ...
    $('#btnShowTip').click(function(event) {
        ...
        if (re.test(original_str)) {
          ...
          orig      = $('#hfDataB2').val().replace(/([\,\.])/g," $1").split(" ");
...
function get_correct_answers() {
  ...
  var correct = new Array();;
  correct.push($('#hfDataB').val().replace(/([\,\.])/g," $1").replace(/-/g," ").replace(/:::/g,"-"));
  if ($('#hfDataB2').length) {
    correct.push($('#hfDataB2').val().replace(/([\,\.])/g," $1").replace(/-/g," ").replace(/:::/g,"-"));
  }
  if ($('#hfDataB3').length) {
    correct.push($('#hfDataB3').val().replace(/([\,\.])/g," $1").replace(/-/g," ").replace(/:::/g,"-"));
  }
  if ($('#hfDataB4').length) {
    correct.push($('#hfDataB4').val().replace(/([\,\.])/g," $1").replace(/-/g," ").replace(/:::/g,"-"));
  }
  if ($('#hfDataB5').length) {
    correct.push($('#hfDataB5').val().replace(/([\,\.])/g," $1").replace(/-/g," ").replace(/:::/g,"-"));
  }
  if ($('#hfDataB6').length) {
    correct.push($('#hfDataB6').val().replace(/([\,\.])/g," $1").replace(/-/g," ").replace(/:::/g,"-"));
  }
